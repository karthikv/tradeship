...
exports.tryJSONParse = function(contents, defaultValue) {
  try {
    return JSON.parse(contents) || defaultValue;
  } catch (err) {
  ...
  return exports
    ...
    .then(files => {
      ...
      return exports.readFile(bestFilePath, "utf8").then(
        contents => parseConfig(bestFilePath, contents),
    ...
    .then(config => {
      ...
      return getEslintConfig(meta, parent);
    });
}
...
function parseConfig(filePath, contents) {
  ...
  switch (path.basename(filePath)) {
    ...
    case ".eslintrc.yaml":
    case ".eslintrc.yml":
      return tryYAMLParse(contents, null);
    ...
    case "package.json":
      return exports.tryJSONParse(contents, {}).eslintConfig || null;

    default:
      throw new Error(`bad eslint file: ${filePath}`);
  }
}
