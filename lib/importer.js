...
const { lint, fileRegex, findPkgMeta } = require("./common");
...
exports.run = function(dir, code, override) {
  ...
  return findPkgMeta(dir).then(meta => {
    ...
    if (config.parserOptions) {
      const { ecmaVersion } = config.parserOptions;
      if (ecmaVersion < 6) {
        const message = "only es6 (es2015) and higher is supported; " +
          `eslint config says you're using es${ecmaVersion}`;
    ...
    return DepRegistry.populate(config, dir, meta).then(depRegistry => {
