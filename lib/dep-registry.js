...
class DepRegistry {
  ...
  populate(dir) {
    return this
      .findPkgMeta(dir)
      .then(() => {
        const promises = [this.readCache()];
        if (this.dir) {
          promises.push(this.findSrcFiles(this.dir));
        }
      ...
      .then(([cache, srcFiles]) => {
        ...
        const promises = srcFiles.map(sf => {
        ...
        return stat(filePath).then(s => {
          ...
          if (s.isFile() && (ext === ".js" || ext === ".jsx")) {
            ...
            return this
              ...
              .then(sfs => srcFiles.push(...sfs));
          }
        });
      });

      return Promise.all(promises).then(() => srcFiles);
    });
  }
