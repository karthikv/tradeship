...
class DepRegistry {
  ...
  static populate(dir, meta) {
    if (this.promises && this.promises[dir]) {
      return this.promises[dir].then(instance =>
        instance.copy(meta.eslintConfig));
    }

    const instance = new DepRegistry(meta.eslintConfig);
    this.promises = this.promises || {};
    this.promises[dir] = instance.populate(meta).then(() => instance);
    return this.promises[dir];
  }

  constructor(eslintConfig = null, registry = null, deps = null) {
  ...
  search(name) {
    ...
    if (dep && !this.eslintConfig.env.node && isNodeID[dep.id]) {
      return null;
    }
    return dep;
  }
  ...
  copy(eslintConfig) {
    return new DepRegistry(eslintConfig, this.registry, this.deps);
  }
  ...
  populate(meta) {
    ...
    return Promise
      .all([this.readCache(cachePath), this.findSourceFiles(meta.root || null)])
  ...
  populateFile(entry, filePath) {
    return readFile(filePath, "utf8")
      ...
      .then(exported => {
        ...
        exported.defaults.forEach(d => entry.defaults.push(d));
        ...
        if (exported.hasDefaults || entry.defaults.length > 0) {
          // assume all idents are actually defaults
